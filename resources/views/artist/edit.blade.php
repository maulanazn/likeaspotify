...
@section('content') 
    ...
    <form action="{{route('artist_update', $artist->name)}}" method="POST" class="container mt-3 g-3">
        @method('PUT')
        @csrf
        <input type="text" class="form-control mb-3" name="name" readonly id="name" value="{{$artist->name}}" placeholder="Name...">
        @error('name')
            <div>{{$message}}</div>
        @enderror

        <textarea name="description" id="description" cols="30" readonly rows="10" placeholder="Description...">{{$artist->description}}</textarea>
        @error('description')
            <div>{{$message}}</div>
        @enderror
